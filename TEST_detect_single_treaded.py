...
if __name__ == '__main__':
    ...
    parser.add_argument('--annotation-folder', type=str,
                                    default='/home/drussel1/data/ADL/object_annotation', help='the path to the data')
    parser.add_argument('--which-object', type=int,
                                    default=0, help='Usually either 0 or 1')
    parser.add_argument('--which-dataset', type=str,
                                    default="ADL", help='Either "ADL", "CUSTOM", "OTHER"')
    ...
    if args.which_dataset == "CUSTOM":
        ...
        video_name = args.video.split('/')[-1].replace('.MP4', '')
        annotation_file = "{}/object_annot_{}.txt".format(args.annotation_folder, video_name)
        print(annotation_file)
        with open(annotation_file, 'r') as readfile:
            """"object_track_id", "x1", "y1", "x2", "y2", "frame number", "1 if the object is active and 0 otherwise", "object label"""
            first_line = readfile.readline().split(" ")
            print(first_line)
            x = int(first_line[1])
            y = int(first_line[2])
            w = int(first_line[3]) - x
            h = int(first_line[4]) - y
            init_bbox = (x, y, w, h)
            print(init_bbox)
